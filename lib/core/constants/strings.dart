/// App strings for localization
/// Arabic (العربية) and English strings
class AppStrings {
  // Private constructor to prevent instantiation
  AppStrings._();

  // App General
  static const String appName = 'قناة علاء حامد';
  static const String appNameEn = 'Alaa Hamed Channel';

  // Splash Screen
  static const String splashSubtitle = 'محتوى تعليمي مميز';
  static const String splashSubtitleEn = 'Premium Educational Content';

  // Videos Page
  static const String allVideos = 'كل الفيديوهات';
  static const String allVideosEn = 'All Videos';
  static const String failedToLoadVideos = 'Failed to load videos';
  static const String retry = 'Retry';
  static const String noVideosAvailable = 'No Videos Available';
  static const String checkBackLater = 'Check back later for new content';
  static const String videosCount = 'فيديو';
  static const String videosCountEn = 'videos';
  static const String loadingMore = 'Loading more...';
  static const String pullToRefresh = 'Pull to refresh';

  // Video Player Page
  static const String publishedOn = 'Published on';
  static const String moreDetails = 'More Details';

  // Auth Page
  static const String googleSignIn = 'Google Sign In';
  static const String signInWithGoogle = 'Sign In with Google';
  static const String signedIn = 'Signed in!';
  static const String accessToken = 'Access Token';
  static const String error = 'Error';
  static const String authenticationFailed = 'Authentication failed';

  // Error Messages
  static const String serverError = 'Server error occurred';
  static const String networkError = 'Network connection failed';
  static const String connectionTimeout = 'Connection timeout';
  static const String noInternetConnection = 'No internet connection';
  static const String unexpectedError = 'An unexpected error occurred';

  // Settings Page
  static const String settings = 'الإعدادات';
  static const String settingsEn = 'Settings';
  static const String appearance = 'المظهر';
  static const String appearanceEn = 'Appearance';
  static const String theme = 'السمة';
  static const String themeEn = 'Theme';
  static const String themeLight = 'فاتح';
  static const String themeLightEn = 'Light';
  static const String themeDark = 'داكن';
  static const String themeDarkEn = 'Dark';
  static const String themeSystem = 'تلقائي';
  static const String themeSystemEn = 'System';
  static const String language = 'اللغة';
  static const String languageEn = 'Language';
  static const String arabic = 'العربية';
  static const String english = 'English';
  static const String notifications = 'الإشعارات';
  static const String notificationsEn = 'Notifications';
  static const String enableNotifications = 'تفعيل الإشعارات';
  static const String enableNotificationsEn = 'Enable Notifications';
  static const String notificationsDescription = 'تلقي إشعارات عند نشر فيديوهات جديدة';
  static const String notificationsDescriptionEn = 'Receive notifications for new videos';
  static const String videoSettings = 'إعدادات الفيديو';
  static const String videoSettingsEn = 'Video Settings';
  static const String videoQuality = 'جودة الفيديو';
  static const String videoQualityEn = 'Video Quality';
  static const String autoPlay = 'تشغيل تلقائي';
  static const String autoPlayEn = 'Auto Play';
  static const String autoPlayDescription = 'تشغيل الفيديو التالي تلقائياً';
  static const String autoPlayDescriptionEn = 'Automatically play next video';
  static const String storage = 'التخزين';
  static const String storageEn = 'Storage';
  static const String clearCache = 'مسح ذاكرة التخزين المؤقت';
  static const String clearCacheEn = 'Clear Cache';
  static const String clearCacheDescription = 'حذف الصور والبيانات المخزنة مؤقتاً';
  static const String clearCacheDescriptionEn = 'Delete cached images and data';
  static const String account = 'الحساب';
  static const String accountEn = 'Account';
  static const String signOut = 'تسجيل الخروج';
  static const String signOutEn = 'Sign Out';
  static const String signOutConfirmation = 'هل أنت متأكد من تسجيل الخروج؟';
  static const String signOutConfirmationEn = 'Are you sure you want to sign out?';
  static const String cancel = 'إلغاء';
  static const String cancelEn = 'Cancel';
  static const String confirm = 'تأكيد';
  static const String confirmEn = 'Confirm';
  static const String about = 'حول التطبيق';
  static const String aboutEn = 'About';
  static const String version = 'الإصدار';
  static const String versionEn = 'Version';
  static const String developer = 'المطور';
  static const String developerEn = 'Developer';
  static const String developerName = 'Mahmoud Ahmed';
  static const String contact = 'تواصل معنا';
  static const String contactEn = 'Contact Us';
  static const String qualityAuto = 'تلقائي';
  static const String qualityAutoEn = 'Auto';
}
